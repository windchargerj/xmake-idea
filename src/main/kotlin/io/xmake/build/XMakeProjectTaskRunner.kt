...
import io.xmake.run.command.awaitSuccessfulCompletion
...
class XMakeProjectTaskRunner : ProjectTaskRunner() {
    ...
    private suspend fun execute(
        ...
        for (task in tasks) {
            for (command in task.commands) {
                progress.output("> ${command.commandLine.commandLineString}\n", ProcessOutputType.SYSTEM)
                command.createProcessHandler(
                    project,
                    XMakeConsoleOptions(showProblems = true, preserveAnsi = true),
                ...
                ).awaitSuccessfulCompletion(command.commandLine.commandLineString)
            }
        }
    ...
    private fun XMakeProblem.filePosition(remote: Boolean): FilePosition? {
        ...
        val resolvedPath = resolveFilePath() ?: return null
