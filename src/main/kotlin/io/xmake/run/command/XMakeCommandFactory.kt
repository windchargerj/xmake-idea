...
internal class XMakeCommandFactory(project: Project, profile: XMakeBuildProfile) {
    ...
    private val configureOptions = commandArguments {
        args("-m", profile.mode)
        option("-p", profile.platform.takeUnless { it == XMakeBuildProfile.DEFAULT_VALUE })
        option("-a", profile.architecture.takeUnless { it == XMakeBuildProfile.DEFAULT_VALUE })
        if (profile.toolchain != XMakeBuildProfile.DEFAULT_VALUE) {
    ...
    fun createBuild(): XMakeCommand = createTargetBuild(XMakeBuildProfile.DEFAULT_VALUE)
    ...
    fun createTargetPathQuery(target: String): XMakeCommand {
        ...
        return createCommand(environmentOverrides = QUERY_ENVIRONMENT) {
            args("l", scriptPath)
            target
                .takeUnless { it == XMakeBuildProfile.DEFAULT_VALUE || it.isBlank() }
                ?.let { args(it) }
        }
    }
    ...
    private fun MutableList<String>.target(value: String) {
        when (value) {
            "all" -> args("-a")
            "", XMakeBuildProfile.DEFAULT_VALUE -> Unit
            else -> args(value)
        }
    }
